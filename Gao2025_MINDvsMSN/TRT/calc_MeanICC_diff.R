## Calculate the Mean ICC difference between methods
...
for (curr_set in c('BNU1', 'HNU1')){
  ...
  for (curr_atlas in c('DK308', 'Schaefer300')){
    ## Load Mean ICC data
    in_fname <- paste(out_dir, '/', curr_atlas, '_MeanICC.rds', sep = '')
    ICC_dat <- readRDS(in_fname)
    ...
    for (pair_idx in c(1:NP)){
      ...
      curr_x_real <- ICC_dat[[curr_x_name]][['Real']]
      curr_y_real <- ICC_dat[[curr_y_name]][['Real']]
      curr_x_jack <- ICC_dat[[curr_x_name]][['Jack']]
      curr_y_jack <- ICC_dat[[curr_y_name]][['Jack']]
      curr_x_boot <- ICC_dat[[curr_x_name]][['Boot']]
      curr_y_boot <- ICC_dat[[curr_y_name]][['Boot']]
    ...
    ## Save
    out_fname <- paste(out_dir, '/', curr_atlas, '_MeanICC_Diff.csv', sep = '')
