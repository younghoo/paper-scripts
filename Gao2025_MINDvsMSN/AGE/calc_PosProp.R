...
## Loop each statistic
for (curr_stat in c('CohenD')){
  ## Loop each dataset
  for (curr_set in c('eNKI', 'Cam-CAN')){
    ## Loop each atlas
    for (curr_atlas in c('DK308', 'Schaefer300')){
      ## Load raw data
      in_fname <- paste('./PROCDATA/STATS/AGE/', curr_set, '/', curr_atlas, '_edge_Sig_CohenD.rds', sep = '')
      ...
      ## Save
      out_fname <- paste('./PROCDATA/STATS/AGE/', curr_set, '/', curr_atlas, '_', curr_stat, '_PosProp.csv', sep = '')
