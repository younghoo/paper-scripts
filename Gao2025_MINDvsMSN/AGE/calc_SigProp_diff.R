## Calculate the significance proportion difference between methods
...
for (curr_set in c('eNKI', 'Cam-CAN')){
  ...
  for (curr_atlas in c('DK308', 'Schaefer300')){
    ## Load SigProp data
    in_fname <- paste(out_dir, '/', curr_atlas, '_SigProp.rds', sep = '')
    Prop_dat <- readRDS(in_fname)
    ...
    for (pair_idx in c(1:NP)){
      ...
      curr_x_real <- Prop_dat[[curr_x_name]][['Real']]
      curr_y_real <- Prop_dat[[curr_y_name]][['Real']]
      curr_x_jack <- Prop_dat[[curr_x_name]][['Jack']]
      curr_y_jack <- Prop_dat[[curr_y_name]][['Jack']]
      curr_x_boot <- Prop_dat[[curr_x_name]][['Boot']]
      curr_y_boot <- Prop_dat[[curr_y_name]][['Boot']]
    ...
    ## Save
    out_fname <- paste(out_dir, '/', curr_atlas, '_SigProp_Diff.csv', sep = '')
